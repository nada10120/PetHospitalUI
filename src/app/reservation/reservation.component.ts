...
export class ReservationComponent implements OnInit {
  ...
  loadPets() {
    this.http.get<any[]>(`https://pethospital.runasp.net/api/Admin/Pets/GetAllByUser/${this.authservice.getCurrentUserId()}`).subscribe({
      next: (data) => (this.pets = data),
      error: (err) => console.error(err),
    });
  }
  loadServices(){
    this.http.get<any[]>(`https://pethospital.runasp.net/api/Admin/Services`).subscribe(
      {
  ...
  onSubmit() {
    ...
    this.http.post(`https://pethospital.runasp.net/api/Admin/Appointments`, reservation).subscribe({
